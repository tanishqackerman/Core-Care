...
public class DoctorListForChat extends AppCompatActivity {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                reference.addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        binding.chatRv.setAdapter(new DoctorPrescriptionAdapter(DoctorListForChat.this, hehe, new ItemClickListener() {
                            @Override
                            public void onItemClick(int position, int check) {
                                if (check == 0) {
                                    startActivity(new Intent(DoctorListForChat.this, ChatPage.class).putExtra("model", hehe.get(position)).putExtra("data", new ChatList(hehe.get(position).getEmail().replace("@gmail.com", "").replace("\\.", ""))));
                                }
                            }
                        }));
                        binding.chatRv.setLayoutManager(new LinearLayoutManager(DoctorListForChat.this));
                    }
